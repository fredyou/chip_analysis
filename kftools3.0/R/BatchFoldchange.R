...
function(DataFile = "data.txt", ContentFile = "Content.txt", BatchInfoFile = "Condition.txt", 
    ...
    for (i in 1:nrow(BatchInfo)) {
        ...
        if (Paired) {
            ...
            # read Flag
            label1Flag <- paste(prefixes, BatchInfo[i, 1], suffixes2, sep = "")
            label2Flag <- paste(prefixes, BatchInfo[i, 2], suffixes2, sep = "")
            ...
            # SampleInfo <- read.delim(file.path(FilePath, ContentFile), sep = '\t', header = T, fill =
            # T,check.names=FALSE)
            ...
            # read Flag
            label1Flag <- paste(prefixes, label1Raw, suffixes2, sep = "")
            label2Flag <- paste(prefixes, label2Raw, suffixes2, sep = "")
