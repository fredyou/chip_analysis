diff_up_down_gene_count <-
function(datafile = c("CS1_VS_M1_p005fc2.txt", "CS1_VS_M1_p001fc2.txt"), filepathin = "./", 
    filepathout = "./") {
    tb_count <- NULL
    for (f_name in datafile) {
        tb1 <- read.delim(paste(filepathin, f_name, sep = ""), sep = "\t", header = T, quote = "", stringsAsFactors = F)
        diff_count <- nrow(tb1)
    ...
    write.table(tb_count_2, file = "diff_up_down_gene_count.xls", sep = "\t", col.names = T, row.names = F, quote = F)
