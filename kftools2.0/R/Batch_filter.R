Batch_filter <-
function(Batch_infofile = "Condition.txt", contentfile = "Content.txt", outfile_suffixes = "_p005", 
    filepath = "./", colname2 = "filename", prefixes = "", suffixes1 = "_NS", suffixes2 = "_flags", P_value = 0.05, 
    is.FC = 1, FC = 2, envelope = 1, is.adjustP = 0, is.mean_sel = 1, mean_sel = 6) {
    ...
    for (i in 1:nrow(Batch_info)) {
        ...
        # read T-test result file outputdir<-paste(filepath,Batch_info[i,1],'_VS_',Batch_info[i,2],sep='')
        outputdir <- filepath
        ...
        if (exists(paste(Batch_info[i, 1], "_VS_", Batch_info[i, 2], sep = ""))) {
            ...
            data <- read.delim(paste(outputdir, "/", filename, ".txt", sep = ""), sep = "\t", quote = "", header = T, 
                fill = T)
        ...
        if (is.mean_sel == 1) {
            ...
            for (group_i in total_group) {
                sample_i <- as.character(group_tb[grep(paste("^", group_i, "$", sep = ""), group_tb[, "Group"], 
                  perl = T), "name"])
        ...
        write.table(Filter, file = paste(outputdir, "/", Batch_info[i, 1], "_VS_", Batch_info[i, 2], outfile_suffixes, 
            ".xls", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
        write.table(Filter, file = paste(outputdir, "/", Batch_info[i, 1], "_VS_", Batch_info[i, 2], outfile_suffixes, 
            ".txt", sep = ""), sep = "\t", quote = FALSE, col.names = TRUE, row.names = FALSE)
    }
}
